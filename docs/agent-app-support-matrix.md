...
| Claude desktop | Desktop-session discovery from the shared transcript store, deep-link session switching, Accessibility prompt delivery, hook-driven turn and permission state | Experimental  | No            | Live Accessibility delivery, permission-dialog control, and result state on a real Claude window are unverified; local contracts pass `pnpm qa:claude-desktop` |
| Claude Code    | PTY launch pinned to a session id, hook-driven status, session resume, and runtime controls  | Experimental  | No            | Authenticated end-to-end prompt/result behavior is unverified; the CLI and desktop cards now coexist by session ownership |
