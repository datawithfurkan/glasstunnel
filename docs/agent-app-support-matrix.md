...
| Claude desktop | Desktop-session discovery from the shared transcript store, front-window verification with Accessibility fallback for switching, Accessibility prompt delivery, hook-driven turn and permission state | Experimental  | No            | The app's `claude://code` links are gated off on current builds, and Accessibility delivery, session switching, and permission-dialog control on a real window are unverified; local contracts and store parsing are recorded |
| Claude Code    | PTY launch pinned to a session id, hook-driven status, session resume, and runtime controls  | Partial       | No            | Adapter-level live prompt/result/Stop-hook evidence is recorded; the phone-driven prompt, permission, interrupt, and relaunch flow through the mobile browser is not yet |
