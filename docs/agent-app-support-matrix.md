...
| Claude desktop | Desktop-session discovery from the shared transcript store, front-session verification (rename control) with Accessibility fallback for switching, Accessibility prompt delivery, hook-driven turn and permission state | Experimental  | No            | The app's `claude://code` links are gated off on current builds; the composer and current-session controls are verified read-only through Accessibility, but prompt delivery, session switching, and permission-dialog control on a real window are not yet recorded (`pnpm lab:e2e:claude-desktop` is ready and needs a dedicated app session) |
