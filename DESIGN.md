# Glasstunnel Design System

## Current Direction: Professional SaaS With Mono Accents

Glasstunnel should feel like a focused remote-coding product, not a debug panel. The core product path is:

**Sign in -> choose Mac -> choose coding app or feature -> continue work.**

Transport is now relay-first. The Mac stays connected to the Glasstunnel relay
even when no browser is open; web/mobile reconnects to the relay and should show
cached workspace state immediately while live state refreshes. WebRTC is no
longer the primary control path and should be treated as an optional future
video transport only.

The UI should be simple enough that a new user knows the next click without reading explanatory paragraphs. Internal protocol state belongs in details, diagnostics, or advanced sections, not in the main product path.

### Rules

- Use a clean system sans-serif as the primary UI voice.
- Use Berkeley Mono only for brand flavor, short labels, codes, terminal content, device IDs inside details, and technical diagnostics.
- Show one primary action per screen or section.
- Do not repeat the same state in multiple places. If the header shows connection state, the sidebar and inspector should not repeat it.
- Do not expose direct local setup, approval requests, or setup timeouts in production UI.
- Avoid primary-path words such as `trusted`, `approval`, `pairing`, `fallback`, `host ID`, and `live` unless the user explicitly opens advanced/diagnostic details.
- Prefer short labels: `Open`, `Add Mac`, `Sign in`, `Sign out`, `View only`, `Connected`, `Offline`.
- Use fewer cards, fewer borders, and fewer status badges. Cards should group decisions, not decorate every datum.
- The Mac app should follow desktop conventions: stable sidebar navigation, detail panes for work, inspectors only when useful, and settings separated from day-to-day workflow.

### Remote-App UI Model

- The Mac `Workspace` is now `Remote apps`: a status and management list for supported coding apps and direct features.
- Do not expose grid, cell, drop-zone, layout, or toggle language in the primary UI. Those concepts are deprecated compatibility internals.
- Remote apps and features are opened from the web/mobile UI on demand. The Mac UI reflects readiness, availability, and sync status rather than asking users to pre-enable anything.
- Screen sharing and Terminal are direct features, not projects. They should open directly into their control surfaces and must not appear as project/thread rows.
- Each row should answer three questions without extra explanation: app or feature name, whether it is available, and what the current context/status is.
- Multiple windows/projects belong inside compact selectors on the app row or inside the active app card, never in a separate catalog panel.
- The web app should show supported coding apps and direct features as a compact switcher. If only one project-capable app is present, avoid redundant tabs and go straight to its projects.
- Preferred status language: `Ready`, `Syncing context`, `App not open`, `Mac offline`, `Connection failed`, `Waiting for screen`.
- Relay status language should be precise: `Online`, `Reconnecting`, `Offline`, `Syncing Codex`, `Waiting for your choice`.
- Avoid showing `RemoteApp`, `AgentID`, `GridLayout`, `adapter`, `position`, or bundle IDs outside diagnostics.

## Visual Reference: OpenCode-Inspired Foundation

The section below is visual inspiration, not an override of the current rules above. Current Glasstunnel product UI uses a clean system sans-serif as the primary voice, with Berkeley Mono reserved for brand flavor, terminal content, codes, short labels, and diagnostics.
...
In the OpenCode reference, Berkeley Mono is the sole typeface. Glasstunnel borrows the terminal-native restraint without copying that rule wholesale: app navigation, prose, and controls should stay system sans-serif unless the current rules above explicitly call for mono.
...
**Key Characteristics:**
- Berkeley Mono-heavy reference style, adapted in Glasstunnel as mono accents rather than monospace everywhere
...
### Font Family
- **Glasstunnel primary UI**: system sans-serif.
- **Glasstunnel mono accents**: `Berkeley Mono`, with fallbacks: `IBM Plex Mono, ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, Liberation Mono, Courier New, monospace`.
...
### Principles
- **Reference note**: OpenCode uses one monospace voice. Glasstunnel should keep clearer product ergonomics by pairing system sans-serif UI with selective mono accents.
...
## 10. Glasstunnel Adaptation

As Glasstunnel evolves from a utility into an account-aware remote workspace product, this design language should be adapted with a few product-specific rules:

- **One shell, two clients**: the macOS host app and the hosted web client should feel like the same product. Shared tokens should cover background, surface, outline, status colors, typography, and corner radius.
- **Compact over verbose**: prefer short status rows, badges, and icon-led summaries over explanatory paragraphs. Long explanations belong in onboarding, not in the main product surfaces.
- **Warm terminal, not glass dashboard**: avoid glossy cards, blur-heavy layers, oversized rounded corners, and gradient-heavy UI. Glasstunnel should feel like a refined terminal workspace, not a generic SaaS control panel.
- **Radius discipline**: default interactive and panel radius should stay in the `4px` to `6px` range. Larger radii should be rare and justified.
- **Account-first trust model**: account sign-in, trusted devices, approvals, and active host state should be visually integrated into the main shell rather than hidden in a setup-only screen.
- **Grid-first hierarchy**: on both macOS and web, the live workspace should remain visually primary. Configuration and trust management should support the workspace, not compete with it.
- **Message surfaces**: chat and agent activity should use left/right grouping, but bubbles should remain compact, rectangular, and monospace-first rather than adopting consumer-chat styling.
